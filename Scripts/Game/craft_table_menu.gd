...
func _controls():
	...
	if Input.is_action_just_pressed("use_map") || Input.is_action_just_pressed("craft_menu"):
		self.visible = false
	...
	if Input.is_action_just_pressed("item_next"):
		...
		if CraftRecipes.craft_table_recipes.size() <= current_recipe_index:
			current_recipe_index = 0
		current_recipe = CraftRecipes.craft_table_recipes[current_recipe_index]
		update_recipe_display()
	
	if Input.is_action_just_pressed("item_previous"):
		current_recipe_index -= 1
		if current_recipe_index < 0:
			current_recipe_index = CraftRecipes.craft_table_recipes.size() - 1
		current_recipe = CraftRecipes.craft_table_recipes[current_recipe_index]
		update_recipe_display()
